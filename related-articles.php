 ...
 * @license   GPL-3.0+
 * @link      https://wordpress.org/plugins/related-articles/
 * @copyright 2020-23 KaizenCoders
 *
 * @wordpress-plugin
 *
 * Plugin Name:       Related Articles
 * Plugin URI:        https://wordpress.org/plugins/related-articles/
...
if ( ! defined( 'KC_RA_MIN_PHP_VER' ) ) {
	define( 'KC_RA_MIN_PHP_VER', '5.6' );
}

if ( ! defined( 'KC_RA_MAX_PHP_VER' ) ) {
	define( 'KC_RA_MAX_PHP_VER', '7.4.22' );
}

if ( ! function_exists( 'kc_ra_fail_php_version_notice' ) ) {

	function kc_ra_fail_php_version_notice() {
		/* translators: %s: PHP version */
		$message      = sprintf( esc_html__( 'Related Articles plugin requires PHP version between %s & %s, plugin is currently NOT RUNNING.', 'shorten-url' ), KC_RA_MIN_PHP_VER,  KC_RA_MAX_PHP_VER);
		$html_message = sprintf( '<div class="error">%s</div>', wpautop( $message ) );
		echo wp_kses_post( $html_message );
	}
}

if ( ! version_compare( PHP_VERSION, KC_RA_MIN_PHP_VER, '>=' ) || ! version_compare( PHP_VERSION, KC_RA_MAX_PHP_VER, '<=' )) {
	add_action( 'admin_notices', 'kc_ra_fail_php_version_notice' );
...
class related_articles_Widget extends WP_Widget {
	...
	function __construct() {
        parent::__construct('related_articles_Widget', 'Related Articles', array('classname' => 'related_articles_Widget', 'description' => 'Display related articles in a widget' ));
    }
	...
	public function widget( $args, $instance ) {
		...
		if (($instance_plugin!==false)&&(is_object($instance_plugin))) {
